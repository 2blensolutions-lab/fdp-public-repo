...
def color_dif(row):
    """Verde si OK, rojo si cualquier diferencia, gris si solo en un sistema."""
    estado = row.get("estado") if isinstance(row, dict) else getattr(row, "estado", None)
    if estado == "OK":          return COLOR_OK
    if estado in ("Solo Payway", "Solo Zetti"): return COLOR_MISSING
    return COLOR_ERROR
...
def conciliar(df_pay: pd.DataFrame, df_zet: pd.DataFrame) -> pd.DataFrame:
    ...
    def estado(row):
        # Ambos en 0 → OK (registro sin importe en ambos sistemas)
        if row["monto_payway"] == 0 and row["monto_zetti"] == 0:
            return "OK"
        if row["monto_payway"] == 0: return "Solo Zetti"
        if row["monto_zetti"]  == 0: return "Solo Payway"
        # Diferencia absoluta menor a $0.01 → OK (diferencias de redondeo)
        if abs(row["diferencia"]) < 0.01: return "OK"
        else:       return "Revisar"
...
def escribir_excel(df: pd.DataFrame, output_path: str, fecha: date):
    ...
    t2 = ws2["A1"]
    t2.value     = "Items para revisar — cualquier diferencia distinta de 0%"
    ...
    df_revisar = df[df["estado"] != "OK"]
    for ri, row in enumerate(df_revisar.itertuples(index=False), 3):
        # Todos en rojo en la hoja Para revisar — son items que requieren atención
        bg = COLOR_ERROR
    ...
    leyenda = [
        (COLOR_OK,      "OK",          "Diferencia exactamente 0% — concilia correctamente"),
        (COLOR_ERROR,   "Revisar",     "Cualquier diferencia distinta de 0% — requiere revisión"),
        (COLOR_MISSING, "Solo Payway", "El movimiento está en Payway pero no en Zetti"),
        (COLOR_MISSING, "Solo Zetti",  "El cupón está en Zetti pero no en Payway"),
    ]
