...
Variables de entorno requeridas:
    ...
    PAYWAY_KEY_1       — private key SSH sociedad 1 (formato OpenSSH)
    PAYWAY_USER_2      — CUIT sociedad 2 (ADB): 30-70792120-6
    PAYWAY_KEY_2       — private key SSH sociedad 2
    PAYWAY_USER_3      — CUIT sociedad 3: 30-67265440-4
    PAYWAY_KEY_3       — private key SSH sociedad 3
"""
...
def _cargar_pkey(key_str: str) -> paramiko.PKey:
    """
    Carga la private key desde un string.
    Normaliza saltos de línea (Railway puede guardarlos como \\n literal)
    y escribe a un archivo temporal para que paramiko la parsee correctamente.
    """
    # Normalizar saltos de línea
    key_str = key_str.replace("\\n", "\n").strip()

    # Escribir a archivo temporal — más robusto que StringIO para claves OpenSSH
    ...
    try:
        ...
        raise paramiko.ssh_exception.SSHException(
            "No se pudo parsear la private key — verificar formato OpenSSH en la variable de entorno."
        )
    finally:
        os.unlink(tmp_path)  # borrar siempre, incluso si hay error
