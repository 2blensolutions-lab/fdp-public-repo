...
def run(fecha: date | None = None) -> tuple[date, list[str]]:
    ...
    for soc in SOCIEDADES:
        ...
        try:
            ...
            print(f"  ⚠️  {soc['nombre']}: sin datos — {e}")
            sin_datos.append(soc["nombre"])

    # Si NINGUNA sociedad trajo datos → día sin actividad, terminar limpio
    if not rutas_locales:
        raise SinDatosException(
            f"Ninguna sociedad tiene datos para el {fecha.strftime('%d/%m/%Y')}. "
            f"Probablemente es un día sin actividad (feriado o fin de semana)."
        )

    # Si ALGUNAS tienen datos → loguear las que no y continuar
    if sin_datos:
        print(f"\n  ⚠️  Sociedades sin datos hoy: {', '.join(sin_datos)}")
        print(f"  Continuando con {len(rutas_locales)} sociedad(es) disponible(s).")
