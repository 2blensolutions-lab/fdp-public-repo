...
Filtros:
  - Estado INGR / INGRESADO + sin anular → monto positivo
  - Estado INGR / INGRESADO + anulado (ANULADO/ANULADOR) → monto negativo
  - Cualquier otro estado → excluir
  - TARJETA DEL PUEBLO, TARJETA MERCADO PAGO QR, TARJETA MODO → excluir siempre
"""
...
ANULADOS_NEGATIVOS = {"ANULADOR"}
TARJETAS_EXCLUIDAS = {"TARJETA DEL PUEBLO", "TARJETA MERCADO PAGO QR", "TARJETA MODO"}
...
def run(fecha: date, output_todos: str, output_resumen: str) -> tuple[str, str, pd.DataFrame]:
    ...
    # CSV completo sin filtrar (auditoría)
    ...
    # 1. INGR válidos → monto positivo
    df_validos = df_todos[
        (df_todos["estado"].str.upper().isin(ESTADOS_VALIDOS)) &
        (df_todos["anulado"].isna())
    ].copy()

    # 2. INGR anulados → monto negativo
    anulado_upper = df_todos["anulado"].fillna("").str.upper()
    df_anulados = df_todos[
        (df_todos["estado"].str.upper().isin(ESTADOS_VALIDOS)) &
        (anulado_upper.isin(ANULADOS_NEGATIVOS))
    ].copy()
    df_anulados = df_anulados.copy()
    ...
    tarjeta_upper = df_procesados["tarjeta_nombre"].fillna("").str.upper()
    mask_excluidas = tarjeta_upper.isin({t.upper() for t in TARJETAS_EXCLUIDAS})
    ...
    print(f"  Tarjetas excluidas:     {excluidas_count} filas")
    print(f"  Total para conciliar:   {len(df_procesados)}")
    print(f"  Tarjetas en resumen:    {sorted(df_procesados['tarjeta_nombre'].dropna().unique())}")

    # -----------------------------------------------------------------------
    # Resumen agrupado por sucursal y tarjeta
    # -----------------------------------------------------------------------
    resumen = (
