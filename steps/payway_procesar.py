...
TERMINALES = {
    ...
    16690652: ("allen", "integrada"), 16711479: ("plottier 2", "desintegrada"),
    16711496: ("plottier 2", "integrada"), 16711884: ("senillosa", "desintegrada"),
    16711499: ("senillosa", "integrada"), 16711476: ("plottier 3", "desintegrada"),
    16711497: ("plottier 3", "integrada"), 16680271: ("central", "desintegrada"),
    16711484: ("senillosa", "desintegrada"), 16726284: ("san martin", "desintegrada"),
...
# ---------------------------------------------------------------------------
def run(rutas_csv: list[str], output_path: str, fecha: date | None = None) -> tuple[str, pd.DataFrame, date]:
    """
    Procesa los CSVs de Payway y genera el Excel.

    rutas_csv:   lista de rutas locales a los 3 CSVs descargados en el paso 1
    output_path: ruta local donde guardar el Excel resultante
    fecha:       fecha a procesar (si se omite, se infiere del CSV — no recomendado)
    ...
    # Si viene por parámetro, usarla siempre (más seguro)
    # Si no viene, inferir del CSV como fallback
    if fecha is not None:
        fecha_date = fecha
        print(f"  Filtrando por fecha recibida por parámetro: {fecha_date.strftime('%d/%m/%Y')}")
    else:
        if not fechas:
            raise ValueError("No se encontraron fechas válidas en los CSVs de Payway.")
        fecha_date = datetime.strptime(fechas[-1], "%Y%m%d").date()
        print(f"  ⚠️  Fecha inferida del CSV (no recomendado): {fecha_date.strftime('%d/%m/%Y')}")

    # Filtrar solo el día correcto — descarta filas coladas de otros días
    df_dia = df[df["fecha_operacion"].dt.date == fecha_date].copy()
    if len(df_dia) == 0:
        raise ValueError(
            f"No hay filas para el {fecha_date.strftime('%d/%m/%Y')} en los CSVs. "
