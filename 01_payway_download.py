...
def descargar_sociedad(
    ...
    """
    Conecta al SFTP, encuentra el archivo de la fecha indicada y lo descarga.

    sociedad:   dict con numero, nombre, usuario, password
    fecha:      date a conciliar
    local_path: ruta local donde guardar el archivo (/tmp/YYYYMMDD_Movimientos_Nde3.csv)

    Retorna local_path si tuvo éxito.
    Lanza Exception si no encuentra el archivo o falla la conexión.
    """
    nombre_soc = sociedad["nombre"]
    usuario    = sociedad["usuario"]
    numero     = sociedad["numero"]

    if not sociedad["password"]:
        raise EnvironmentError(
            f"PAYWAY_PASS_{numero} no está configurada en las variables de entorno."
        )
    ...
    try:
        transport.connect(username=usuario, password=sociedad["password"])
