...
def main(fecha_override: date | None = None):
    """
    fecha_override: si se pasa, corre el pipeline para esa fecha específica
                    en lugar de ayer. Útil para reprocesar días anteriores.
    """
    print("=" * 60)
    ...
    if fecha_override:
        print(f"  ⚠️  Modo reproceso: fecha forzada a {fecha_override.strftime('%d/%m/%Y')}")
    ...
    try:
        fecha_run, rutas_csv = payway_download.run(
            fecha=fecha_override + timedelta(days=1) if fecha_override else None
        )
        ...
        # Caso esperado — domingo, feriado, sin actividad
        print(f"\n  ℹ️  {e}")
        ...
        sys.exit(0)  # exit 0 → Railway no lo marca como crash
    ...
    payway_xlsx = None
    try:
        # fecha_datos = ayer respecto al run (o la fecha forzada)
        # Se pasa explícitamente para evitar que filas coladas de otro día
        # confundan al procesador (bug del 13/05: CSV del 12 traía datos del 11)
        fecha_datos = fecha_override if fecha_override else (fecha_run - timedelta(days=1))
        ...
        # Renombrar todo con la fecha correcta de los datos
        ...
        rutas_renombradas = []
        total = len(rutas_csv)
        for i, (_, ruta_vieja) in enumerate(archivos_generados, 1):
    ...
    try:
        mail_exito(
            fecha=fecha_str,
            resumen=resumen,
            adjunto_path=conc_path if conc_path and os.path.exists(conc_path) else None,
            adjunto_payway=payway_xlsx if payway_xlsx and os.path.exists(payway_xlsx) else None,
        )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print(f"Corriendo pipeline para fecha: {fecha_arg.strftime('%d/%m/%Y')}")
        main(fecha_override=fecha_arg)
    else:
        main()
